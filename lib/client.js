...
function receivedChatMessage(messageData) {
    ...
    logger.info('(not init) receivedChatMessage:' + messageData);

    if (!initialized) return;

    logger.info('receivedChatMessage:' + messageData);
...
PlugAPI.prototype.connect = function(roomSlug) {
    if (!roomSlug || typeof roomSlug !== 'string' || roomSlug.length === 0 || roomSlug.indexOf('/') > -1) {
        if(roomSlug == null)
        {
            console.log("roomSlug is null");
            throw new Error('null roomSlug: ' + roomSlug);
        }
